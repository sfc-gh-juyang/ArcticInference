...
for length_label in length_group_labels:
    ...
    for sys in systems:
        ...
        for i_gpu, gpu_name in enumerate(gpus):
            ...
            cost_per_trillion_tokens = cost_per_gpu[gpu_name] / (
...
def plot_throughput(data_dict, figname):
    n_gpus = len(gpus)
    ...
    ax.set_ylabel("Throughput (K tokens/s)", fontweight="bold")
    plt.text(
        x=-0.2,
        y=1600,
        s="Seq len 50",
    ...
    ax.annotate(
        ...
        color=colors[3],  # Text color matching the arrow
    )
    ...
    ax.annotate(
        ...
        horizontalalignment="center",  # Position text to the right of arrow
        verticalalignment="center",  # Center text vertically
        color=colors[4],  # Text color matching the arrow
    )
    ...
    ax.annotate(
        ...
        horizontalalignment="center",  # Position text to the right of arrow
        verticalalignment="center",  # Center text vertically
        color=colors[5],  # Text color matching the arrow
    )
...
def plot_cost(data_dict, figname):
    systems = ["before optimization", "after optimization"]
    n_gpus = len(gpus)
    n_systems = len(systems)
    n_length_groups = len(length_group_labels)
    ...
    legend_handles = {}  # To store unique handles for the legend (systems only)
    current_primary_x_group_idx = 0
    colors = ["#5B5B5B", "#29B5E8",     "#D45B90",  # 16x
    "#FF9F36",
    "#7254A3"
]
    ...
    for length_label in length_group_labels:
        ...
        for i_gpu, gpu_name in enumerate(gpus):
            ...
            for i_sys, sys_name in enumerate(systems):
                ...
                if sys_name == "before optimization":
                    value = data_for_current_length["vllm"][i_gpu]
                else:
                    value = data_for_current_length["Arctic Inference"][i_gpu]
    ...
    # --- Setup Axes and Legend ---
    ax.set_ylabel("Cost ($/Trillion tokens)", fontweight="bold")
    plt.text(
        x=line_x - 0.48,
        y=38 * 1000,
        s="Seq len 50",
    ...
    ax.annotate(
        "16x",
        xy=(1.08, 1600),  # Upper point
        xytext=(1.08, 34800),  # Lower point
        textcoords="data",
        xycoords="data",
        arrowprops=dict(
            arrowstyle="<->",
    ...
    ax.annotate(
        "4.2x",
        xy=(4.08, 1600),  # Lower point
        xytext=(4.08, 9000),  # Upper point
        textcoords="data",
        xycoords="data",
        arrowprops=dict(
            arrowstyle="<->",
...
if __name__ == "__main__":
    plot_throughput(throughput_data_by_length, "embedding_throughput.png")
    plot_cost(cost_data_by_length, "embedding_cost.png")
